// Author: shahil_005
// Time: 2021-04-29 02:19:49
// Problem: C. Heap Operations
// Contest: Codeforces - Codeforces Round #357 (Div. 2)
// URL: https://codeforces.com/contest/681/problem/C
// Memory Limit: 256 MB
// Time Limit: 1000 ms
// 
...
void solve(int tc)
{
		...
		if(s=="insert"){
			...
			if(sz(pq)){
				...
				if(y==x){
					...
					while(sz(pq) && (ll)pq.top()<x){
						pq.pop();
						v.pb({"removeMin",0});
					}
					if(sz(pq)==0 || (ll)pq.top()>x){
						pq.push(x);
						v.pb({"insert",x});
					}
					v.pb({s,x});
	...
	//If command "remove" : just check if sz(heap)>0, if not insert any x
	//If command "getMin" : #check if sz(heap)>0, if not insert that particular x
	//			#if sz(heap)>0 already, check if min(pq)==x, if yes continue
	//						      if min(pq)>x : insert that x
	//						      if min(pq)<x : add removeMin command until min(pq)>=x
