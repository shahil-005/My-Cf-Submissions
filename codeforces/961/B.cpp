...
void solve()
{
	...
	if(k>=n){
		...
		fe(i,1,n){
			pr[i]+=pr[i-1];
			if(!b[i]){pr[i]+=a[i];}	//Only including sleeping time cases
			else{ans+=a[i];}	
		}	
		LL add=0;
		fe(i,k,n){
			LL x=pr[i]-pr[i-k];	//This way it gets sum of all previous k elements inclding current index only adding elements where he is sleeping
			//trace2(i,x);
			add=max(add,x);
		}
		cout<<ans+add<<endl;
