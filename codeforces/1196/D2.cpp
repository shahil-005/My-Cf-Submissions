...
// URL : https://codeforces.com/contest/1196/problem/D1
// Memory Limit : 256 MB
// Time Limit : 2000 ms
...
void solve()
{
	...
	mem(c,0);
	fe(i,1,n){
		a[i]+=a[i-1]+(s[i-1]!=x1[i-1]);
	}
	fe(i,1,n){
		b[i]+=b[i-1]+(s[i-1]!=x2[i-1]);
	}
	fe(i,1,n){
		c[i]+=c[i-1]+(s[i-1]!=x3[i-1]);
	}
	LL y1=INT_MAX,y2=INT_MAX,y3=INT_MAX;
	fe(i,k,n){
		LL temp=a[i]-a[i-k];
		y1=min(y1,temp);
	}
