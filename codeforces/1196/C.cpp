...
// Problem : C. Robot Breakout
// Contest : Codeforces - Codeforces Round #575 (Div. 3)
// URL : https://codeforces.com/contest/1196/problem/C
// Memory Limit : 256 MB
// Time Limit : 3000 ms
...
void solve()
{
      LL n;
      cin>>n;
      map<LL,pair<pair<LL,LL>,pair<LL,LL> > > m;
      fe(i,1,n){
      		LL x,y;
      		cin>>x>>y;
      		LL a,b,c,d;
      ...
      fe(i,1,n){
      		//cout<<i<<" : "<<m[i].ff.ff<<" "<<m[i].ff.ss<<" "<<m[i].ss.ff<<" "<<m[i].ss.ss<<endl;
      		x1=max(x1,m[i].ff.ff);
      		x2=min(x2,m[i].ff.ss);
      		x3=min(x3,m[i].ss.ff);
      		x4=max(x4,m[i].ss.ss);
