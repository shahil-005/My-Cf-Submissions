...
void solve(int tc)
{
	...
	//Run dfs from 1 
	//Find such node f such that the below subtrees don't satisfy the coloring condition
	//Run  dfs from that node f and check if its above subtree satisfies that condition
	//If yes, then that node f is good and has to be used as the root 
}
int main(){
	start();
	time__("solve"){
