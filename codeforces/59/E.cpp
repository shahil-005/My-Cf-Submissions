...
void solve(int tc)
{
	...
	while(!q.empty()){
		...
		q.pop();
		for(auto it:adj[nd]){
			if(s.find({par,{nd,it}})!=s.end()){
				continue;
			}
			if(dis[nd][it]==0){
				dis[nd][it]=dis[par][nd]+1;
				pre[nd][it]=par;
				q.push({nd,it});
				// debug(par,nd,it,dis[par][nd],dis[nd][it],pre[nd][it]);
			}
		}
	}
	// for(int i=1;i<=n;i++){
