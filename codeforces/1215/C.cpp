...
void solve()
{
	...
	//trace2(n1,n2);
	if((n1%2)&&!(n2%2)){cout<<-1;}
	else if(!(n1%2)&&(n2%2)){cout<<-1;}
	else{
		if(n1%2==0&&n2%2==0){
			f(i,0,n1){
				swap(s[v1[i]],t[v1[i+1]]);
				ans.pb({v1[i]+1,v1[i+1]+1});
				i++;
			}
			f(i,0,n2){
				swap(s[v2[i]],t[v2[i+1]]);
				ans.pb({v2[i]+1,v2[i+1]+1});
				i++;
			}
		}
		else{
			...
			ans.pb({v1[n1-1]+1,v1[n1-1]+1});
			ans.pb({v1[n1-1]+1,v2[n2-1]+1});
		}
		cout<<ans.size()<<endl;
		auto(ans){
