...
LL fun(LL x,LL y,LL z){
	return (power(abs(x-y),2) + power(abs(y-z),2) + power(abs(z-x),2)) ; 
}
void solve2(VL a,VL b,VL c){
	each(it,a){
		auto p=lower_bound(all(b),it);
		auto q=upper_bound(all(c),it);
		if(p==b.end() || q==c.begin()){
			continue;
		}
		q--;
		ans=min(ans,fun(it,*p,*q));
	}
}
void solve(LL tc)
{
