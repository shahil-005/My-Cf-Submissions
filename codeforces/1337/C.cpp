...
VL adj[N];
LL level[N];	//depth of node
LL sub[N];	//subtree size of node
bool vis[N];
void dfs(LL s,LL p){
	vis[s]=true;
	for(auto it:adj[s]){
...
void solve(LL tc)
{
	//ref : https://www.youtube.com/watch?v=6IY9kPbzkAI&ab_channel=takeUforward
	//contribution of each node = (level of that node) - (subtree size of that node)
	...
	fe(i,1,n){
		// d2(level[i],sub[i]);
		LL c=level[i]-sub[i];	//contribution of node
		v.pb(c);
	}
	sort(all(v));
	reverse(all(v));
