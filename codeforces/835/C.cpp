...
// vector<vector<vector<ll> >> v( 101 , vector<vector<ll> > (101));
ll a[101][101][12];
ll row[101][101][12],col[101][101][12];
void solve(int tc)
{
	//2d prefix sum
	ll n,q,c;
	...
	for(int k=0;k<=10;k++){
		for(int i=1;i<=100;i++){
			for(int j=1;j<=100;j++){
				row[i][j][k]=row[i][j-1][k]+a[i][j][k];
			}
		}
	}
	
	// ll time;cin>>time;
	// for(int k=time;k<=time;k++){
		// for(int i=1;i<=10;i++){
			// for(int j=1;j<=10;j++){
				// cout<<a[i][j][k]<<" ";
			// }
	...
	while(q--){
		...
		ll ans=0;
		for(int i=x1;i<=x2;i++){
			ans+=(row[i][y2][t]-row[i][y1-1][t]);
		}
		cout<<ans<<endl;
	}
	
	//we will main a 2d prefix sum of each row in row[][]
	//total sum of brightness at any time can be calculated the prefix sum matrix for those co-ordinates
	//time will get repeated after an interval, this interval will be (c+1)
	//after each (c+1), the whole a[][] matrix will repeat itself, so  t=t%(c+1);
