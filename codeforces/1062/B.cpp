...
// URL: https://codeforces.com/contest/1062/problem/B
// Memory Limit: 256 MB
// Time Limit: 1000 ms
...
bool powerof2(LL n){if(n==0){return false;}return (ceil(log2(n)) == floor(log2(n)));}
void solve(LL tc)
{
	LL n;
	cin>>n;
	...
	else if(powerof2(maxv) && f==1){
		cout<<ans<<" "<<1+(LL)log2(maxv);
	}
	else if(powerof2(maxv)){
		cout<<ans<<" "<<(LL)log2(maxv);
	}
	else{
		if(f==1){
			cout<<ans<<" "<<2+(LL)log2(maxv);
