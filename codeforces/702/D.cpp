...
void solve(LL tc)
{
	...
	if(d<=k){
		...
		if(d%k==0){	//When for last repairing, k is left to travel
			LL x=d/k;
			cout<< (x-2)*t + (x-1)*(k*a) + min(t+k*a,b*k);
		}
		else{	//When for last repairing, <k is left to travel
			LL x=d/k;
			LL y=d%k;
			cout<< (x-1)*t + x*(k*a) + min(t+y*a,b*y);
		}
	}
	else{
		cout<<a*k + (d-k)*b;
