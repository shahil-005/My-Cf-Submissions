...
// URL: https://codeforces.com/contest/349/problem/B
// Memory Limit: 256 MB
// Time Limit: 2000 ms
...
void solve(LL tc)
{
	//First find the maximum number of digits that can be possible
	//Use all n for max number of digits && choose the highest number(digit) but there are multiple choices
	
	//If some n is left after that,utilise that n to increase the value of the highest place digit
	//But now preference should be given to using highest value at the highest place instead of multiple small values like in the previous step
