...
// URL: https://codeforces.com/contest/370/problem/A
// Memory Limit: 256 MB
// Time Limit: 1000 ms
...
void solve(LL tc)
{
	...
	LL op1 = abs(a-c) + abs(abs(abs(a-c)+b)-d);
	LL op2 = abs(a-c) + abs(abs(abs(a-c)-b)-d);
	LL op3 = abs(b-d) + abs(abs(abs(b-d)+a)-c);
	LL op4 = abs(b-d) + abs(abs(abs(b-d)-a)-c);
