// Author: shahil_005
// Time: 2021-05-04 07:44:17
// Problem: B. Petya and Exam
// Contest: Codeforces - Codeforces Round #425 (Div. 2)
// URL: https://codeforces.com/contest/832/problem/B
// Memory Limit: 256 MB
// Time Limit: 2000 ms
// 
	...
	while(q--){
		...
		if(f!=-1){
			if(sz(b)-1>sz(s)){
				...
				int z=n-x-1;
				int y=n1-x-z;
				string b1=b.substr(0,x);
				string b2=b.substr(f+1,z);
// 				
				
				string s1=s.substr(0,x);
				string s2=s.substr(x,y);
				string s3=s.substr(x+y,z);
				// if(s=="vsdduqahk"){
					// debug(b,s);
					// debug(f,x,y,z);
					// debug(b1,b2);
					// debug(s1,s2,s3);
				// }
				if(check(b1,s1) && check(b2,s3) && check2(s2)){
					cout<<"YES"<<endl;
				}
				else{
					cout<<"NO"<<endl;
