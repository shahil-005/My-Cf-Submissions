...
void solve()
{
	LL n;
	cin>>n;
	//cout<<((powm(x,n-2,MOD)*powm(n-2,x,MOD))+MOD)%MOD;
	//cout<<(powm(x,n-2,MOD)*powm(n-2,x,MOD));
	//cout<<(power(x,n-2)*(1<<(n-2)))%MOD;
	//cout<< fact(n,MOD)- powm(2,n-1,MOD);
	LL x=fact(n,MOD)- powm(2,n-1,MOD);
	cout<<(x+MOD)%MOD;
}
int main()
{
