// Author: shahil_005
// Time: 2021-04-30 08:39:22
// Problem: A. Perfect Pair
// Contest: Codeforces - Codeforces Round #188 (Div. 1)
// URL: https://codeforces.com/contest/317/problem/A
// Memory Limit: 256 MB
// Time Limit: 1000 ms
// 
...
void solve(int tc)
{
	...
	if(y<=0){	//the pairs can never increase as both nos are -ve
		cout<<-1;
		return;
	}
	ll cnt=0;
	if(x<0){	//editorial help : here x<0 && y>0, so we can't do brute force as it will be too slow
		ll z=abs(x)/y;
		cnt+=z;
		z*=y;
		x+=z;
	}
	while(max(x,y)<m){	//after that brute forcely increase the pairs, the growth will be fast(see editorial)
		if(x>y){
			swap(x,y);
		}
		x+=y;	//we will increase the smaller no(basic logic)
		cnt++;
	}
	cout<<cnt;
}
