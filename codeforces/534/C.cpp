...
void solve(int tc)
{
	...
	for(int i=1;i<=n;i++){
		ll x=max(0ll,A-d[i]);	//finding range of possible A with given constraints of each i
		ll y=max(0ll,A-1);
		
		ll X=n-1;		//finding actual range possible without considering d[i] values	
		ll Y=sum-d[i];
		
		if(y<X || x>Y){		//No intersection, so no values satisfies for given constraints of d[i]
			cout<<d[i]<<" ";
		}
		else{			//taking intersection such that this final [x,y] will be possible values which can come 
			x=max(x,X);
			y=min(y,Y);
			cout<<d[i]-(y-x+1)<<" ";	//all other values will not satisfy, so this will be the answer
		}
	}
