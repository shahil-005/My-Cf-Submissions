...
void solve(int tc)
{
	...
	//For all (X%Y), take all the Ys at one place
	//For each y, it will be (1%y)^(2%y)^(3%y)^(4%y).....(n%y)
	//So, (1^2^3^4^...y-1) will be repeated (n/y) times
	//if (n/y) is even, it will contribute 0,else it will contribute (1^2^3^4...y-1)
	//also (1^2^3^....y-1) will also get contributed one more time upto index (n%y)
	//pre array is used to store all prefix xors
