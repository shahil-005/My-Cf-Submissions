...
void solve(LL tc)
{
	...
	LL min1=imax,min2=imax,min3=imax;
	fe(i,1,x){cin>>a[i];s1+=a[i];min1=min(min1,a[i]);}
	fe(i,1,y){cin>>b[i];s2+=b[i];min2=min(min2,b[i]);}
	fe(i,1,z){cin>>c[i];s3+=c[i];min3=min(min3,c[i]);}
	sum=s1+s2+s3;
	
	LL ans1=sum-2*min({s1,s2,s3});
	
	LL arr[3]={min1,min2,min3};
	sort(arr,arr+3);
	// f(i,0,3){
		// cout<<arr[i]<<" ";
	// }
