...
void solve(LL tc)
{
	//Same as : https://codeforces.com/contest/1443/problem/B
	LL n,a,b;
	cin>>n>>a>>b;
	S s;
	cin>>s;
	...
	if(pos==-1){
		...
		// d1(cnt);	//This tells the number of barriers bw 0s 
		if(a<b){	//Then just connect all 0s with each other by performing operation 1 and in last use op2 to convert all of them
			cout<<(cnt)*a + b;
		}
		else if(a>=b){	//Then just convert each substrings of 0s at their own place using op2
			cout<<(cnt+1)*b;
		}
	}
}
