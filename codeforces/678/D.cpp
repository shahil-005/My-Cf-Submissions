...
// LL mul(LL a, LL b,LL mod){  return (a * 1LL * b)%mod;}
...
void solve(LL tc)
{
	...
	cin>>a>>b>>n>>x;
	if(a==1){
		cout<<sum(x,mul(n,b,MOD),MOD);
	}
	else{
		LL t1=powm(a,n,MOD);
		LL t2=mul(x,t1,MOD);
		
		LL t3=powm(a-1,MOD-2,MOD);	//modulo inverse using Fermat's theorem'
		LL t4=mul(b,t1-1,MOD);
		t4=mul(t4,t3,MOD);
