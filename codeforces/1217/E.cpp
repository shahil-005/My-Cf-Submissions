...
node merge(node x,node y){
	node res;
	fe(j,0,10){
		LL temp[]={x.d[j].ff,x.d[j].ss,y.d[j].ff,y.d[j].ss};
		sort(temp,temp+4);
		res.d[j].ff=temp[0];
		res.d[j].ss=temp[1];
		res.ans=min(res.ans,res.d[j].ff+res.d[j].ss);
	}
	return res;
}
...
node query(LL s,LL e,LL i,LL x,LL y){
	if(x<=s && e<=y){
		return tree[i];
	}
	else if(y<s || x>e || s>e){
		...
		LL mid=(s+e)/2;
		node left=query(s,mid,2*i,x,y);
		node right=query(mid+1,e,(2*i)+1,x,y);
		
		return merge(left,right);
	}
	
}
void update(LL i,LL s,LL e,LL ind,LL val){
	if(s==e){
		a[s]=val;
		LL temp=val;
		fe(j,0,10){
		...
		LL mid=(s+e)/2;
		if(ind<=mid){
			update(2*i,s,mid,ind,val);
		}
		else{
			update((2*i)+1,mid+1,e,ind,val);
		}
		tree[i]=merge(tree[2*i],tree[(2*i)+1]);
	}
}
void solve(LL tc)
{
// https://codeforces.com/blog/entry/69554?#comment-540811
...
// https://codeforces.com/blog/entry/69554?#comment-540954
	...
	//Editorial Approach ,build a seg tree with each node containing 2 parameters
	//Review it again in future
	//Rough work for sample test case : https://ibb.co/yyyqVvH
