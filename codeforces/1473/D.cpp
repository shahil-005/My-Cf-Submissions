...
void solve(LL tc)
{
	//Second thread : https://youtu.be/jlMIIK92zog?t=7921
	...
	LL pmax[n+1],pmin[n+1],psum[n+1];
	LL smax[n+1],smin[n+1];
	mem(pmax,0);
	mem(pmin,0);
	mem(psum,0);
	mem(smax,0);
	...
	// fe(i,1,n){cout<<psum[i]<<" ";}cout<<endl;
	// fe(i,1,n){cout<<pmax[i]<<" ";}cout<<endl;
	// fe(i,1,n){cout<<pmin[i]<<" ";}cout<<endl;
	// fe(i,1,n){cout<<smax[i]<<" ";}cout<<endl;
	// fe(i,1,n){cout<<smin[i]<<" ";}cout<<endl;
	fe(i,1,m){
		LL l,r;
		cin>>l>>r;
		
		LL up=pmax[l-1];	//Max up on left segment
		LL down=pmin[l-1];	//Min down on left segment
		
		LL up2=psum[l-1]+(smax[r]-psum[r]);	//Max up on right segment
		LL down2=psum[l-1]-(psum[r]-smin[r]);	//Min down on right segment
		
		// d4(up,down,up2,down2);
		LL maxup=max(up,up2);	//Max value touched
		LL maxdown=min(down,down2);	//Min value touched
		
		cout<<maxup-maxdown+1<<endl;
	}
}
int main()
{
