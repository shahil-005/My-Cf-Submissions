...
//g++ -static -DONLINE_JUDGE -Wall -Wextra -pedantic -std=c++17 -O3 -Wshadow -Wformat=2 -Wfloat-equal -Wconversion -Wlogical-op -Wshift-overflow=2 -Wduplicated-cond -Wcast-qual -Wcast-align -D_GLIBCXX_DEBUG -D_GLIBCXX_DEBUG_PEDANTIC
...
void solve(LL tc)
{
	...
	if(n==1){
		...
		// fe(i,1,n){cout<<b[i]<<" ";}cout<<endl;
		// fe(i,1,n){cout<<c[i]<<" ";}cout<<endl;
		fe(i,1,q){
			LL l,r;
			cin>>l>>r;
			...
			if(l==r){
				...
				LL ans1=a[l+1]-1-1;
				LL ans2=k-a[r-1]-1;
				cout<<ans1+ans2+b[l+1]<<endl;
			}
			else{
				LL ans1=a[l+1]-1-1;
				LL ans2=k-a[r-1]-1;
				cout<<ans1+ans2+c[r-1]-c[l]<<endl;
			}
