// Author: shahil_005
// Time: 2021-04-22 20:51:11
// Problem: C. Dasha and Password
// Contest: Codeforces - Codeforces Round #394 (Div. 2)
// URL: https://codeforces.com/contest/761/problem/C
// Memory Limit: 256 MB
// Time Limit: 2000 ms
// 
...
void solve(int tc)
{
	...
	int dp[n+1][4];
	for(int i=1;i<=n;i++){
		dp[i][1]=dp[i][2]=dp[i][3]=51;
		for(int j=1;j<=m;j++){
			if(a[i][j]>='0' && a[i][j]<='9'){
				dp[i][1]=min(dp[i][1],min(j-1,m+1-j));
			}
			else if(a[i][j]>='a' && a[i][j]<='z'){
				dp[i][2]=min(dp[i][2],min(j-1,m+1-j));
			}
			else{
				dp[i][3]=min(dp[i][3],min(j-1,m+1-j));
			}
		}
