...
void solve(LL tc)
{
	//We used bitmasking approach to select all possible combination in the array
	LL n,l,r,x;
	cin>>n>>l>>r>>x;
	fe(i,1,n){
		cin>>a[i];
	...
	LL ans=0;
	for(LL mask=1;mask<(1<<n);mask++){//For eg n=5,then we only consider upto 31(not 32)
		if(popc(mask)>=2){	//No of elements in each combination should be atleast 2
			VL v;
			fe(j,0,n-1){	//the highest bit will be at (n-1)th position(0-based indexing),So,we only need to shift n-1 times
				if((mask>>j)&1){
					v.pb(a[j+1]);
				}
			}
			LL sum=0;
			sort(all(v));
			LL dif=abs(v[0]-v.back());
			for(auto it:v){
				sum+=it;
			}
			if(sum>=l && sum<=r && dif>=x){
				ans++;
			}
		}
	}
