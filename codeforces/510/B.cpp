// Author: shahil_005
// Time: 2021-03-09 13:00:37
// Problem: E. Fox And Two Dots
// Contest: Codeforces - Practice 9th March
// URL: https://codeforces.com/gym/319508/problem/E
// Memory Limit: 256 MB
// Time Limit: 2000 ms
// 
...
char a[51][51];
bool vis[51][51];
void dfs(ll i,ll j,ll px,ll py,ll x,ll y,bool &f){
	// if(x==1 && y==1){
		// debug(i,j,f);
	// }
	if(i<1 || j<1 || i>n || j>m){
	...
	vis[i][j]=true;
	for(int k=0;k<4;k++){
		if(i+fx[k]!=px || j+fy[k]!=py){
			dfs(i+fx[k],j+fy[k],i,j,x,y,f);
		}
		if(f){
			return;
		}
