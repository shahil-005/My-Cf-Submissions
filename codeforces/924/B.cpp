...
void solve(int tc)
{
	...
	for(int i=1;i<=n-2;i++){
		...
		if(a[k]-a[i]<=u && k-i>=2){
			// debug(i,k);
			ld cur=(a[k]-a[i+1])/(a[k]-a[i]);
			ans=max(ans,cur);
		}
	}
	cout<<fix(ans,10);
	
	//The equation can be converted to 1 - (a[j]-a[i])/(a[k]-a[i])
	//for every i, we need to find the rightmost k such that a[k]-a[i]<=u as a[k] needs to be min
	//for every i, j=i+1 as a[j] needs to be min
}
int main(){
	start();
	time__("solve"){
